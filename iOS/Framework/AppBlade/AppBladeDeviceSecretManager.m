...
@implementation AppBladeDeviceSecretManager
...
- (BOOL)isDeviceSecretBeingConfirmed
{
    ...
    if(tokenRequestInProgress) { //the queue has a maximum concurrent process size of one, that's why we can do what comes next
        AppBladeWebOperation *process = (AppBladeWebOperation *)[[[[AppBlade sharedManager] tokenRequests] operations] objectAtIndex:0];
