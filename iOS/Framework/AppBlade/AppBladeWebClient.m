...
//NSOperation related
@property (nonatomic, assign, readwrite) BOOL appbladeFinished;
@property (nonatomic, assign, readwrite) BOOL appbladeExecuting;
...
- (void) issueRequest
{
    if((nil != self.request) && !self.isCancelled){
            ...
            self.appbladeExecuting = YES;
            [self didChangeValueForKey:@"isExecuting"];
            [self willChangeValueForKey:@"isFinished"];
            self.appbladeFinished = NO;
            [self didChangeValueForKey:@"isFinished"];
...
// Flags
- (BOOL)isExecuting {
    return self.appbladeExecuting;
}

- (BOOL)isFinished {
    return self.appbladeFinished;
}
